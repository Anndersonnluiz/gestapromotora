...
@Named
@ViewScoped
public class DemaisFGTSMB implements Serializable {
	...
	public void gerarListaDemaisOperacoes(int situacao) {
		...
		String sql = "Select c From Contrato c WHERE c.tipooperacao.descricao like '%FGTS%'"
	...
	public String alterarSituacao(Contrato contrato) {
		...
		HttpSession session = (HttpSession) fc.getExternalContext().getSession(false);
		session.setAttribute("contrato", contrato);
		session.setAttribute("voltar", "consDemaisFGTS");
	...
	public void pesquisar() {
		String sql = "Select c From Contrato c WHERE c.tipooperacao.descricao like '%FGTS%' and c.cliente.nome like '%"
	...
	public void gerarListaInicial() {
		ContratoFacade contratoFacade = new ContratoFacade();
		String sql = "Select c From Contrato c WHERE c.tipooperacao.descricao like '%FGTS%'"
				+ "and c.simulacao=false  and c.ultimamudancasituacao>='2021-01-01'";
